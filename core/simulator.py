...
def analyze_simulation_results(results: List[int], target_featured: int = 1) -> Dict:
    ...
    if n == 0:
        return {
            "simulations": 0, "target_featured": target_featured,
            "average": 0, "median": 0, "min": 0, "max": 0,
            "prob_table": {}, "percentiles": {}, "raw_results": [],
        }
    ...
    prob_table = {t: sum(1 for r in results if r <= t) / n for t in thresholds}
    ...
    return {
        ...
        "average": round(avg, 2),
        "median": median,
        "min": results_sorted[0],
        "max": results_sorted[-1],
