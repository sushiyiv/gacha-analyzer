...
class MihoyoGachaFetcher(BaseFetcher):
    ...
    def fetch_records(self, url: str = None, account_id: int = None,
        ...
        try:
            ...
            error_msg = str(e)
            if any(k in error_msg.lower() for k in ["authkey", "expired", "过期"]):
                raise FetcherError(
                    self.expired_tip or f"authkey 已过期，请重新获取。\n原始信息：{error_msg}"
                )
            raise FetcherError(error_msg)

        self._report_progress("正在解析记录...", 0.8)
        records = [
            MihoyoAPI.parse_record(raw, self.game_key, account_id or 0)
