...
class StarRailFetcher(BaseFetcher):
    """星穹铁道抽卡记录获取器"""
    ...
    def fetch_records(self, url: str = None, account_id: int = None, latest_time: str = None) -> List[GachaRecord]:
        ...
        if not url:
            ...
            url = self.cache.extract_url("starrail")
            if not url:
                raise FetcherError(
                    "无法自动获取URL。\n\n"
                    "请按以下步骤操作：\n"
        ...
        self._report_progress("正在获取跃迁记录...", 0.3)

        try:
            raw_records, detected_uid = self.api.fetch_all("starrail", url, self._report_progress, latest_time, cancel_check=self._cancel_check)
        ...
        for raw in raw_records:
            record = MihoyoAPI.parse_record(raw, "starrail", account_id or 0)
